...
ResNet18 モデルを用いて white データセットのテスト用フォルダに含まれる画像ごとに Grad-CAM を適用し、
各画像ごとの可視化結果を保存するスクリプト。

機能概要:
- white ディレクトリ直下の config.yaml からパス設定を読み込み
- 学習済み ResNet18 モデルをロード
...
arch = "resnet18"
model_path = root / paths["models_dir"] / "resnet18_looks_classifier.pth"
image_root = root / paths["image_root"]
out_root   = root / paths["gradcam_dir"] / arch
folders    = ["normal_test", "good_test"]

run_gradcam_on_folders(
    arch, model_path, image_root, out_root,
    folders, num_classes=2, img_size=224
)