...
model_path = "/home/student/e21/e215706/dm/sorce/hyper/model/custom_hyper.pth"
...
for folder in target_folders:
    ...
    output_path = os.path.join(output_base, f"gradcam_custom_hyper_average_{folder}.png")
    plt.figure(figsize=(6, 6))
    plt.imshow(avg_heatmap, cmap='jet')
    plt.colorbar()
    plt.title(f"Average Grad-CAM(Hyper): {folder}")
    plt.axis('off')
    plt.tight_layout()
...
output_all_path = os.path.join(output_base, "gradcam_custom_hyper_average_all.png")
plt.figure(figsize=(6, 6))
plt.imshow(avg_all, cmap='jet')
plt.colorbar()
plt.title("Average Grad-CAM(Hyper): All (good + normal)")
plt.axis('off')
plt.tight_layout()
